...
const AUTHORIZATION_PATH = '/ccm/authorize';
const TOKEN_PATH = '/ccm/token';
const REGISTER_PATH = '/ccm/register';
const REVOKE_PATH = '/ccm/revoke';
const CONSENT_PATH = '/ccm/oauth/consent';
const MCP_PATH = '/ccm/mcp';
...
function renderConsentPage(request) {
  ...
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CCM authorization</title></head><body>' +
    '<main><h1>CCM authorization</h1>' + clientLabel +
    '<p>Enter the local approval secret to allow this client to access CCM tools.</p>' +
...
function appendHttpTrace(runtime, message) {
  try {
    fs.appendFileSync(path.join(runtime.config.logDir, 'ccm-http.log'), '[' + new Date().toISOString() + '] ' + message + '\n', 'utf8');
  } catch {}
}
...
function buildAuthorizationServerMetadata(config) {
  return {
    ...
    authorization_endpoint: config.issuer + AUTHORIZATION_PATH.slice('/ccm'.length),
    token_endpoint: config.issuer + TOKEN_PATH.slice('/ccm'.length),
    registration_endpoint: config.issuer + REGISTER_PATH.slice('/ccm'.length),
    revocation_endpoint: config.issuer + REVOKE_PATH.slice('/ccm'.length),
...
function loadApprovalSecret(config) {
  ...
  if (typeof secret !== 'string' || secret.length < 16) {
    throw new Error('CCM approval secret is missing or too short.');
  }
  return secret;
}

export function createCcmOAuthServer(options = {}) {
  ...
  const server = http.createServer((request, response) => {
    ...
    handleRequest(request, response, runtime).catch((error) => {
      ...
      logMessage(logger, 'error', 'CCM self-host sidecar request failed.');
    });
  });
...
export function listenCcmOAuthServer(runtime, port = runtime.config.port, host = runtime.config.host) {
  return new Promise((resolve, reject) => {
...
export async function closeCcmOAuthServer(runtime) {
...
async function main() {
  ...
  await listenCcmOAuthServer(runtime);
  logMessage(
    console,
    'log',
    'CCM self-host sidecar listening on ' + config.host + ':' + config.port + '.',
  );
...
if (currentFile === invokedFile) {
  main().catch(() => {
    process.stderr.write('CCM self-host sidecar failed to start. Check the CCM log.\n');
    process.exitCode = 1;
  });
}
