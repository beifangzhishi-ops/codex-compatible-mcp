...
export class OAuthStore {
  ...
  load() {
    ...
    try {
      ...
      throw new Error('CCM OAuth state file is invalid.');
    }
    ...
    if (
      ...
      throw new Error('CCM OAuth state file has an unsupported format.');
    }
  ...
  registerClient(metadata) {
    ...
    const clientId = createOpaqueValue('ccm_client_');
    const clientName =
  ...
  createAuthorizationCode({
    ...
    this.pruneAndSave();
    const code = createOpaqueValue('ccm_code_');
  ...
  mintTokenPair({ clientId, resource, scope, tokenTtlSeconds, familyId = null }) {
    this.pruneAndSave();
    const accessToken = createOpaqueValue('ccm_at_');
    const refreshToken = createOpaqueValue('ccm_rt_');
    const tokenFamilyId = familyId || createOpaqueValue('ccm_family_');
    const now = this.clock();
