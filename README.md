...
The direct MCP surface is intentionally kept small and stable. New ordinary capabilities should default to the **Deferred + Code Mode** surfaces and be invoked through `tool_search` + `exec`. Add a new top-level Direct tool only when the capability is a common operational primitive or is fundamental to environment/project discovery, explicit approval, process continuation, host-native file binding, or nested-tool discovery/dispatch. `list_projects` is Direct + Code Mode because it is the single discovery entry point for connected environments and registered projects. `exec` remains Direct because it is the nested dispatcher and also carries the optional top-level native ChatGPT file binding used by deferred file consumers. `create_projectless_context`, `select_workspace`, and `register_workspace` remain Deferred; approval rendering is centralized in the Direct-only `request_approval` bridge.
...
Do not promote a capability to Direct merely for convenience. Keep workspace discovery/context/lifecycle helpers deferred, and prefer deferred `ccm-extra.*` or other namespaced capabilities for specialized workflows. `apply_patch` and `view_image` remain common Direct operational tools. `ccm-extra.send_file` and `ccm-extra.receive_file` are Deferred + Code Mode capabilities discovered through `tool_search` and invoked through `exec`. Native ChatGPT input-file binding is centralized on the optional top-level `exec.file` parameter instead of keeping `receive_file` itself Direct.
...
CCM ships optional Windows workflows ported from WCM. Specialized workflows, including file handoff, are Deferred + Code Mode capabilities discovered through `tool_search` and invoked through `exec`:

- `ccm-extra.send_file` is a strictly single-file Deferred + Code Mode handoff: one nested call transfers one exact file from a selected CCM environment as a standard MCP `resource_link`. `exec` passes that resource link through unchanged so ChatGPT/the host can materialize and present its native file attachment UI. If multiple files are needed, invoke `send_file` sequentially once per file and wait for each call to return before starting the next; do not issue concurrent/parallel `send_file` calls. Use it only when a user-facing handoff is actually needed (download/open/upload to another tool). CCM stores the exact bytes in the bounded Controller-side `ccm-file:///...` bridge under ignored `.state/file-transfers`; the returned resource link points to that bridge, which persists across Controller restarts, has no time-based expiry by default, and remains subject to configured TTL or cache eviction. After a successful call, the final assistant response should include the host-generated native ChatGPT file attachment object, not its file ID as text. For host download reliability, CCM documents **1 KiB (1024 bytes)** as the operational minimum: smaller files are still transferred byte-for-byte, but some ChatGPT clients may remain stuck connecting when downloading them. CCM never pads, rewrites, or otherwise changes a file to reach 1 KiB; callers should warn the user instead. `send_file` has no CCM file-card App and does not upload the file to ChatGPT Library. Do not use it merely for model-side inspection when the file can be read or viewed locally in CCM; prefer local reading, `view_image`, command-line inspection, or temporary local previews. The transfer does not use BMG.
- `ccm-extra.receive_file` is the inverse Deferred + Code Mode handoff. Its nested `file` argument remains the strict native object (`download_url`, `file_id`, and optional MIME/name); CCM does not resolve file IDs, URIs, placeholders, sandbox paths, or JSON strings into files. For a real conversation or authorized Library attachment, pass that file through the **top-level `exec.file` parameter**. ChatGPT performs native file binding on that top-level field, then CCM injects the already-resolved object into the single nested `receive_file` call before normal schema validation. The selected Worker streams the temporary HTTPS URL directly into its current context root. `destination` is workspace-relative only, overwrite is opt-in, redirects and resolved targets are SSRF-checked, and the completed file is published from a same-directory temporary file with SHA-256 metadata. Multiple files must be received sequentially with one `exec` call per file. ChatGPT Files owns Library discovery; CCM only receives the specific file already selected/authorized by ChatGPT and does not enumerate the user's Library, route bytes through BMG, or use the Controller `FileTransferStore`.
...
Discover deferred specialized workflows with `tool_search` (for example, `send_file`, `receive_file`, `quark upload`, or `bilibili`) and invoke them through `exec`. For a real ChatGPT input file, bind exactly one file through `exec.file` and make exactly one nested call; CCM injects that native file object as the nested call's `arguments.file`. Long uploads/downloads may return a live process session; continue that session with the top-level `write_stdin` tool.
...
For restricted profiles, workspace entry/registration uses the same card capability and one-shot state model, but it never grants `full-access` and never creates an execution allow rule. On approval CCM re-reads the registered workspace or re-inspects the registration path, verifies that the frozen environment/workspace/root still match, then creates the registered `workspace_context`. If the target has drifted, the frozen action fails closed and is consumed so a new approval is required. The model-facing contract treats a pending real-project workspace action as an in-progress project selection: after calling `request_approval`, the model waits for that action to resolve rather than continuing the project task through a projectless context. In a `full-access` environment, `select_workspace` and `register_workspace` perform the same identity/path revalidation and context creation directly without creating an approval request.
