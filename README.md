...
- `ccm-extra.send_file` is a Direct-only, strictly single-file ChatGPT handoff: one call transfers one exact file from a selected CCM environment into the current conversation as a conversation-scoped ChatGPT file. If multiple files are needed, invoke `send_file` sequentially once per file and wait for each call to return before starting the next; do not issue concurrent/parallel `send_file` calls. Use it only when a user-facing handoff is actually needed (download/open/upload to another tool). CCM first stores the exact bytes in the bounded Controller-side `ccm-file:///...` bridge under ignored `.state/file-transfers`; the handoff App reads that internal bridge and calls ChatGPT file upload with `library:false`, so the file is attached to the conversation without being saved to ChatGPT Library. The bridge persists across Controller restarts, has no time-based expiry by default, and remains subject to configured TTL or cache eviction. `structuredContent.resource_uri` is an internal handoff URI, not a user-facing download entry. Do not use `send_file` merely for model-side inspection when the file can be read or viewed locally in CCM; prefer local reading, `view_image`, command-line inspection, or temporary local previews. The transfer does not use BMG.
...
Restricted Workers support an explicit one-shot escalation flow through `exec_command(sandbox_permissions=require_escalated)`. The model supplies the exact command, `workspace_context`, optional working directory/shell/TTY settings, output/yield settings, and user-facing justification once. If no trusted Git/package-script rule or saved workspace policy applies, CCM freezes those fields into a `PendingAction`, assigns an `approval_id` and `operation_id`, and returns without executing the command. The model then calls the Direct-only `request_approval` tool with only that opaque `approval_id`; `request_approval` binds the pending action to the approval card but cannot change the frozen command or execution context.
