...
test('research PPT pipeline exposes the latest user-validated workflow', async () => {
  const runtime = fakeRuntime();
  const registry = new ToolRegistry();
  registerSpecializedTools(registry, runtime);

  const tool = registry.get('ccm-extra.research_ppt_pipeline');
  assert.ok(tool);
  const result = await tool.handler({ section: 'handoff', mode: 'imagegen_assisted' });
  assert.equal(result.isError, undefined);
...
test('research PPT pipeline keeps internal workspace layout flexible and requires image inspection', async () => {
  ...
  const result = await registry.get('ccm-extra.research_ppt_pipeline').handler({
    section: 'source_assets',
  });
  assert.equal(result.isError, undefined);
  assert.match(result.content[0].text, /contact sheet/);
  assert.match(result.content[0].text, /Step 4/);
  assert.match(result.content[0].text, /Imagegen/);
});

test('research PPT pipeline requires full-pack self-audit and mandatory whole-deck user review', async () => {
  const runtime = fakeRuntime();
  const registry = new ToolRegistry();
  registerSpecializedTools(registry, runtime);

  const selfAudit = await registry.get('ccm-extra.research_ppt_pipeline').handler({
    section: 'self_audit',
  });
  const finalReview = await registry.get('ccm-extra.research_ppt_pipeline').handler({
    section: 'final_review',
  });
  assert.match(selfAudit.content[0].text, /production-ready/);
  assert.match(finalReview.content[0].text, /Whole-deck Review/);
  assert.match(finalReview.content[0].text, /Gate/);
});
