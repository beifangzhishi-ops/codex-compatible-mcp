...
function createProtocolServer(
  ...
  const server = new McpServer(SERVER_INFO, {
    ...
    instructions: [
      ...
      'If no real project is selected, discover ccm.create_projectless_context with tool_search and invoke it through exec. Specify environment_id there when a particular Worker is required; omit it to use the primary environment. Do not register a temporary directory merely to get an execution context.',
      ...
      'For multi-step CCM work, prefer exec for deferred/Code Mode capabilities such as workspace discovery/selection, apply_patch, view_image, and ccm-extra tools. Use parallel=true only for independent calls.',
