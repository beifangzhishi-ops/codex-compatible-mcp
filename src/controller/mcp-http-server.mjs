...
function createProtocolServer(
  ...
  const server = new McpServer(SERVER_INFO, {
    ...
    instructions: [
      ...
      'Commands run under the selected environment permission profile. list_environments exposes independent sandbox_read_scope and sandbox_write_scope values. Treat execution context, read scope, and write scope separately: when sandbox_read_scope=host, absolute-path reads outside the selected/projectless context root should be attempted normally and must not trigger select_workspace/register_workspace or escalation merely because the path is outside that root. sandbox_write_scope=context_root means ordinary writes remain limited to the active workspace/projectless root unless an explicit escalation flow authorizes otherwise. Individual file tools may impose narrower path contracts than exec_command.',
      ...
      'When a workspace-write command genuinely requires full-access, call exec_command with sandbox_permissions=require_escalated and an optional justification. If no trusted rule or saved workspace policy applies, CCM freezes the exact command and returns approval_required=true without executing it.',
