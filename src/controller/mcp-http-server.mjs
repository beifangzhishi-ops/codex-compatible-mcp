...
function createProtocolServer(
  ...
  const server = new McpServer(SERVER_INFO, {
    ...
    instructions: [
      ...
      'When the user explicitly asks to plan, discuss before implementation, or re-plan and a durable multi-turn Plan is useful, discover ccm.plan_patch / ccm.plan_read through tool_search. plan_patch carries Codex-style planning discipline: inspect discoverable facts without tracked-state mutation, resolve user intent/tradeoffs, then resolve implementation/test details until the Plan is decision-complete. CCM does not maintain a Plan-Mode state machine. Once the user explicitly asks to execute/implement, continue implementation; plan_read is a neutral reference lookup and never switches the workflow back to planning.',
