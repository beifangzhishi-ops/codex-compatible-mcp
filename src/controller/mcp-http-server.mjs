...
function createProtocolServer(
  ...
  const server = new McpServer(SERVER_INFO, {
    ...
    instructions: [
      ...
      'Use list_projects to discover connected environments and their registered projects. Omit environment_id to inspect every connected environment; pass all=true only when existing projectless contexts also need to be listed.',
      ...
      'If an operational tool requires workspace_context and the user has not selected, requested, or otherwise indicated a real registered project, discover ccm.create_projectless_context with tool_search and invoke it through exec. Do not use projectless as a fallback while select_workspace or register_workspace is awaiting approval. Specify environment_id when a particular Worker is required; omit it to use the primary environment. Use the returned workspace_context for subsequent CCM operations.',
