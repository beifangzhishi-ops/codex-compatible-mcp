export const RESEARCH_PPT_PIPELINE_VERSION = '2026-09-22';
...
3. 通过 CCM 的文件传输能力（优先 ccm-extra.send_file）把需要上传给 Imagegen 的实际参考图片文件发送给用户。
4. 不要求用户去内部工作区找图，也不让用户自己判断应该选哪张。
5. 不把 .md 文件本身作为用户的生成输入附件。
6. 将该页完整、自包含的最终提示词直接在聊天里用独立的 fenced text/code block 给出，方便用户一键复制。
7. 提示词必须是完整版本，不使用“在上一版基础上修改”“沿用前页”等增量指令。
8. 在文本框旁简要注明本页需要上传哪些已发送的参考图；不需要参考图时明确说明无需上传。
9. 用户实际操作：新开 Imagegen 会话 → 上传 GPT 发来的该页参考图 → 复制文本框里的完整提示词 → 粘贴并生成。`,

  generation_review: `## 10–12. Fresh-chat 生成、可选逐页审、动态升级
...
11. 逐页审核不是生产 gate
- 用户可以逐页返图，也可以连续生成很多页甚至全部生成后再看。
- 若用户返图，GPT 优先拦截科学硬伤：公式、数字、物理方向、因果、关键图像误读等。
- 普通间距、字号、轻微对齐等不要求每页都强制返工。
- 不要求用户逐页批准后才能继续下一页。
