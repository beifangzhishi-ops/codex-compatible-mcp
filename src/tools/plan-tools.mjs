...
export function registerPlanTools(registry, runtime) {
  registry.register({
    ...
    description: [
      ...
      'Planning guidance: when the user asks to plan, discuss, or re-plan before implementation, first ground the plan in discoverable facts with targeted non-mutating inspection; then resolve user intent, success criteria, scope, constraints, and material tradeoffs; then resolve implementation approach, interfaces/data flow, edge cases/failure modes, tests/acceptance criteria, and compatibility decisions until another executor could implement without redesigning the solution. Keep the durable Plan updated as decisions change.',
      'During planning, do not perform repo-tracked implementation changes merely because the requested end state is imperative. If the user has already explicitly switched to implementation, patching an existing Plan does not switch the workflow back to planning. CCM itself has no Plan-Mode state machine.',
    ].join('\n\n'),
    inputSchema: {
